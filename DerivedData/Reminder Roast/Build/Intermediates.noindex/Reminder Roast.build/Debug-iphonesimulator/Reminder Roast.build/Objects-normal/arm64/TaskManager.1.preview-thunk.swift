...
#sourceLocation(file: "/Users/daneohr/Downloads/Xcode Projects/Reminder Roast/Reminder Roast/Reminder Roast/TaskManager.swift", line: 1)
import Foundation
import SwiftUI
...
class TaskManager: ObservableObject {
    ...
    private func createDefaultLists() {
        createList(name: __designTimeString("#4600_0", fallback: "Personal"), icon: __designTimeString("#4600_1", fallback: "person"), color: __designTimeString("#4600_2", fallback: "blue"))
        createList(name: __designTimeString("#4600_3", fallback: "Work"), icon: __designTimeString("#4600_4", fallback: "briefcase"), color: __designTimeString("#4600_5", fallback: "red"))
        createList(name: __designTimeString("#4600_6", fallback: "Shopping"), icon: __designTimeString("#4600_7", fallback: "cart"), color: __designTimeString("#4600_8", fallback: "green"))
    }
        ...
        for (listId, sectionTasks) in tasks {
            for (_, tasks) in sectionTasks {
                if tasks.contains(where: { $0.id == taskId }) {
                    return lists.first(where: { $0.id == listId })?.name ?? __designTimeString("#4600_9", fallback: "Unknown List")
                }
            }
        }
        return __designTimeString("#4600_10", fallback: "Unknown List")
    }
    ...
    var todayTasks: [Task] {
        ...
        let tomorrow = calendar.date(byAdding: .day, value: __designTimeInteger("#4600_11", fallback: 1), to: today)!
